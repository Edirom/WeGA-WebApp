...
let $errorText404 := 
    if($lang eq 'en') then (
        ...
        <p>The page you have requested could not be found. If it seems to be an error within our site please do not hesitate to inform us via email at <span onclick="javascript:decEma('{$encryptedBugEmail}')" class="ema">{wega:obfuscateEmail(wega:getOption('bugEmail'))}</span> so we can fix it as soon as possible.</p>,
...
let $logToFile := wega:logToFile('error', $logMessage)
