...
return
    ...
    else if($function eq 'showToolsTab' and wega:getOption('environment') eq 'development') then
        let $docType := request:get-parameter('docType','common')
        return dev:showToolsTab($docType, $lang) 

    else if($function eq 'generateID' and wega:getOption('environment') eq 'development') then
        let $docType := 
            if($lang eq 'en') then request:get-parameter('docType','common')
            else doc(wega:getOption(concat('dic_', $lang)))//entry[lower-case(.) eq lower-case(request:get-parameter('docType', 'Personen'))]/string(@xml:id)
        return dev:createNewID($docType) 

    else if($function eq 'validateIDs' and wega:getOption('environment') eq 'development') then
        let $docType := 
            if($lang eq 'en') then request:get-parameter('docType','common')
            else doc(wega:getOption(concat('dic_', $lang)))//entry[lower-case(.) eq lower-case(request:get-parameter('docType', 'Personen'))]/string(@xml:id)
        return dev:validateIDs($docType)
    
    else if($function eq 'validatePNDs' and wega:getOption('environment') eq 'development') then
        let $docType := 
            if($lang eq 'en') then request:get-parameter('docType','common')
            else doc(wega:getOption(concat('dic_', $lang)))//entry[lower-case(.) eq lower-case(request:get-parameter('docType', 'Personen'))]/string(@xml:id)
        return dev:validatePNDs($docType)
    
    else if($function eq 'validatePaths' and wega:getOption('environment') eq 'development') then
        let $docType := 
            if($lang eq 'en') then request:get-parameter('docType','common')
            else doc(wega:getOption(concat('dic_', $lang)))//entry[lower-case(.) eq lower-case(request:get-parameter('docType', 'Personen'))]/string(@xml:id)
