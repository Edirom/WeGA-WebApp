...
(:    let $response := wega:dictionaryLookup(concat('_', $key), 'persNamesFile'):)
    let $dictionary := wega:getNormDates('persons') 
        (:doc(config:get-option('persNamesFile')):)
...
declare function wega:createLetterNormDates() {
    let $docType := 'letters'
(:    let $normDatesFile := config:get-option(concat($docType, 'NormDatesFile')):)
    let $coll := collection(config:get-option($docType))//tei:TEI[not(./tei:ref)]
...
declare function wega:createWorkNormDates() {
    let $docType := 'works'
(:    let $normDatesFile := config:get-option('worksNormSeriesFile'):)
    let $coll := collection(config:get-option($docType))//mei:mei
...
declare function wega:createDiaryNormDates() {
    let $docType := 'diaries'
(:    let $normDatesFile := config:get-option(concat($docType, 'NormDatesFile')):)
    let $coll := collection(config:get-option($docType))//tei:ab[not(./tei:ref)]
    let $xmlID := concat($docType, 'NormDates')
    let $content := 
        <dictionary xmlns=""> {
            attribute xml:id {$xmlID},
...
(:    let $normDatesFile := config:get-option(concat($docType, 'NormDatesFile')):)
    let $collPath := config:get-option($docType)
    let $predicates := config:get-option(concat($docType, 'PredIndices'))
    let $coll :=  (:collection(config:get-option($docType))//tei:biblStruct[not(./tei:ref)]:)
...
declare function wega:createPersonNormDates() {
    let $docType := 'persons'
(:    let $normDatesFile := config:get-option('persNamesFile'):)
    let $coll :=  collection(config:get-option($docType))//tei:person[not(./tei:ref)]
    let $xmlID := 'persNames'
    let $content := 
        <dictionary xmlns=""> {
            attribute xml:id {$xmlID},
...
declare function wega:getNormDates($docType as xs:string) as document-node()? {
    let $normDatesFileName := 
        if($docType eq 'persons') then config:get-option('persNamesFile') 
        else if($docType eq 'works') then config:get-option('worksNormSeriesFile') 
        else config:get-option(concat($docType, 'NormDatesFile'))
    ...
    return 
        if($updateNecessary) then (
            ...
            return 
                if(exists($newNormDates)) then doc(util:catch('*', xmldb:store($folderName, $fileName, $newNormDates), core:logToFile('error', string-join(('wega:getNormDates', $util:exception, $util:exception-message), ' ;; '))))
                else ()
        )
