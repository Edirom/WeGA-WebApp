    ...
    if($date/@when)
        then if($date/@when castable as xs:date) 
            then $date/string(@when)
                ...
                else string(date:getCastableDate($date/data(@when), $latest))
        else if($latest)
            then if($date/@notAfter)
                then if($date/@notAfter castable as xs:date)
                    then $date/string(@notAfter)
                    else string(date:getCastableDate($date/data(@notAfter), $latest))
                else if($date/@notBefore)
                    then if($date/@notBefore castable as xs:date)
                        then $date/string(@notBefore)
                        else string(date:getCastableDate($date/data(@notBefore), $latest))
                    else if($date/@to)
                        then if($date/@to castable as xs:date)
                            then $date/string(@to)
                            else string(date:getCastableDate($date/data(@to), $latest))
                        else if($date/@from)
                            then if($date/@from castable as xs:date)
                                then $date/string(@from)
                                else string(date:getCastableDate($date/data(@from), $latest))
                            else ()
...
declare function date:parse-date($input as xs:string) as element(tei:date)* {
    ...
    let $pdr-result := wega-util:http-get(xs:anyURI(string-join(($webservice-url, $text), '&amp;')))//pdr:result
    return 
        if($pdr-result) then 
...
declare function date:printDate($date as element(tei:date)?, $lang as xs:string) as xs:string? {
    if($date) then (
        ...
        let $myDate := 
            if($date/@when) then 
                ...
                if(exists($notAfter)) then 
                    if(year-from-date($notBefore) eq year-from-date($notAfter)) then 
                        ...
                        else lang:get-language-string('dateBetween', (replace(date:strfdate($notBefore,$lang,$dateFormat), '(,\s+)?' || year-from-date($notBefore), ''), date:getNiceDate($notAfter,$lang)), $lang) (: Zwischen 1. Juli 1789 und 4. August 1789 :)
                ...
                if(exists($to)) then 
                    if(year-from-date($from) eq year-from-date($to)) then 
                        ...
                        else lang:get-language-string('fromTo', (replace(date:strfdate($from,$lang,$dateFormat), '(,\s+)?' || year-from-date($from), ''), date:strfdate($to,$lang,$dateFormat)), $lang) (: Vom 1. Juli bis 4. August 1789 :)
            ...
            else if(normalize-space($date) castable as xs:date) then date:getNiceDate(xs:date(normalize-space($date)), $lang)
            else lang:get-language-string('dateUnknown', $lang)
        return 
